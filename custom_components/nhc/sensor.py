        ...
        try:
            async with session.get(url, timeout=10) as response:
                if response.status == 200:
                    ...
                    # Safely defaults to an empty string if "binNumber" is missing
                    atlcount = sum(
                        1 for storm in storms_list 
                        if str(storm.get("binNumber", "")).upper().startswith("AT")
                    )
                    epaccount = sum(
                        1 for storm in storms_list 
                        if str(storm.get("binNumber", "")).upper().startswith("EP")
                    )
                    cpaccount = sum(
                        1 for storm in storms_list 
                        if str(storm.get("binNumber", "")).upper().startswith("CP")
                    )
                    self._attr_extra_state_attributes = {"activeStorms": storms_list, "atlantic_storms": atlcount, "eastern_pacific_storms": epaccount, "central_pacific_storms": cpaccount}
                else:
...
class NHCChildTemplateSensor(SensorEntity):
    ...
    def __init__(self, name, unique_id, state_template, attribute_templates):
        ...
        self._attr_name = name
        self._attr_unique_id = unique_id
        self._attr_icon = "mdi:cloud-search" # Base fallback icon
